...
function findSeniors(persons) {
  const webDevSeniors = persons.filter((person) => person.experience >=5 && person.job ==="web dev");
  const dataAnalystSeniors = persons.filter((person)=>person.experience >=5 && person.job==="data analyst")
